...
public class GirlCutScene : MonoBehaviour
{
    ...
    private readonly (bool Man, string Phraze)[] firstDialog = new[]
    {
        ...
        (false, "Наверно, но верить в это сложно... Ладно, по поводу того, как отсюда уйти - уже никак"),
        (false, "Проектировщики бункера сделали так, что все выходы закупориваются, и ни уйти, ни выйти"),
        (false, "Так что да, мы в ловушке"),
        (true, "И че даже никакой вентиляции нет или другого способа выбраться?"),
        (false, "Нууууу... есть конечно один, он может не сработать, но сначала освободи меня"),
        (false, "Короче, на кухне сидит бабиджон, и я не могу выбраться"),
    ...
    private readonly (bool Man, string Phraze)[] secondDialog = new[]
    {
        ...
        (false, "А, да, точно. Короче, как я уже говорила - никак, но я была в команде по уничтожению вируса"),
        ...
        (false, "Она создавалась для правительственных чуваков, и где-то там, во второй части, оно должно быть"),
        (false, "Попасть туда можно через портал, но к нему нужно электричество"),
        (false, "Как запустить машину, ты поймешь сам"),
        (false, "Отправляешься на 7 месяцев назад, пока вирус не распространился"),
    ...
    {
        (false, "Чтобы запустить электричество, тебе надо найти 3 детали")
    };
